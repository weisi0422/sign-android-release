...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const releaseAlias = core.getInput('releaseAlias');
            ...
            const aliasDecoded = Buffer.from(releaseAlias, 'base64').toString();
            console.log(`Test releaseAlias ${releaseAlias}`);
            console.log(`Test releaseAliasDecoded ${aliasDecoded}`);
                ...
                for (let releaseFile of releaseFiles) {
                    ...
                    if (releaseFile.name.endsWith('.apk')) {
                        if (keyStoreDirectory == '') {
                            signedReleaseFile = yield signing_1.signApkFile(releaseFilePath, signingKey, aliasDecoded, keyStorePassword, keyPassword);
                        } else {
                        ...
                        if (keyStoreDirectory == '') {
                            signedReleaseFile = yield signing_1.signAabFile(releaseFilePath, signingKey, aliasDecoded, keyStorePassword, keyPassword);
                        } else {
                            signedReleaseFile = yield signing_1.signAabFile(releaseFilePath, keyStoreDirectory, aliasDecoded, keyStorePassword, keyPassword);
                        }
                    }
                    else {
